...
def set_pending(session_id: str, perspectives: list) -> None:
    """残りのテイル観点で pending を上書きする。空なら pending をクリアする。

    バッチ分割生成で、1回の continue が一部だけ生成したあと、残りを次回以降の
    continue で生成できるよう pending を更新するために使う。
    """
    if perspectives:
        pending_raw = json.dumps({"perspectives": perspectives}, ensure_ascii=False)
    else:
        pending_raw = None
    db.restore_quiz_session_pending(session_id, pending_raw)
